...
export const TOKEN_KEY = 'sso.kiwiui.com'

export const TOKEN_DOMAIN = 'kiwiui.com'

// 全局配置
axios.defaults.baseURL = config.backend

export const setToken = (token) => {
  Cookies.set(TOKEN_KEY, token, {domain: TOKEN_DOMAIN})
}
 ...
 * @returns {Array}
 */
export const getMenuByRouter = (list, access, pPath) => {
  let res = []
  forEach(list, item => {
    if (!item.meta || (item.meta && !item.meta.hideInMenu)) {
      let obj = {
        ...
        name: item.name,
        meta: item.meta,
        path: pPath && (`${pPath}/${item.path}`) || item.path
      }
      if ((hasChild(item) || (item.meta && item.meta.showAlways)) && showThisMenuEle(item, access)) {
        obj.children = getMenuByRouter(item.children, access, obj.path)
      }
...
/**
 * 通过后端获取菜单列表
 * @param {Array} list 通过后端api接口获取的菜单列表
 * @author xiongzhao1
 */
export const getMenuByBackend = (list) => {
...
/**
 * @param {Array} list 获取菜单树
 * @returns {Array}
 * @author xiongzhao1
 */
export const getMenuTree = (list) => {
  return config.useBackendMenu && getMenuByBackend(list) || getMenuByRouter(list)
}
...
export const requestJson = (url, opt) => {
  ...
  return axios(url, opt).then(r => {
    ...
    if (data.code === 301 || data.code === 401) {
      ...
      error({content: data.message, duration: 5})
 ...
 * @param {Object} value
 */
export const findByArray = (arr, key, value) => {
  if (!arr) return null
